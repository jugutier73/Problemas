...
// IngresarLogico devuelve el boolean ingresado por el usuario como respuesta a una pregunta o falso si es un valor inválido.
...
// IngresarOpcion devuelve el entero ingresado por el usuario como respuesta a una pregunta con múltiples opciones del 1 a un máximo especificado o uno si es un valor inválido.
func IngresarOpcion(pregunta string, maximaOpcion int) int {
	opcion := IngresarEntero(pregunta)

	if opcion < 1 || opcion > maximaOpcion {
		MostrarError("La opción no es válida, se asume 1\n\n")
		opcion = 1
	}
