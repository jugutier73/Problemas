...
func main() {
	...
	cantidadEstudiantes := modulo.ContarSegunCriterio(personasLaboratorio,
		tenerTipo, ESTUDIANTE)

	cantidadDocentes := modulo.ContarSegunCriterio(personasLaboratorio,
		tenerTipo, DOCENTE)

	cantidadTecnicos := modulo.ContarSegunCriterio(personasLaboratorio,
		tenerTipo, TECNICO)
...
func set[T comparable](xs []T) map[T]struct{} {
	conjunto := make(map[T]struct{}, len(xs))
