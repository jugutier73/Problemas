...
function main() {
	...
	let cantidadEstudiantes = contarSegunCriterio(
		personasLaboratorio, tenerTipo, ESTUDIANTE);

	let cantidadDocentes = contarSegunCriterio(
		personasLaboratorio, tenerTipo, DOCENTE);

	let cantidadTecnicos = contarSegunCriterio(
...
function obtenerPersonasLaboratorio(ingresos, salidas) {
  ...
  return ingresos.filter(ingreso => !salidasSet.has(ingreso.documento));
}
